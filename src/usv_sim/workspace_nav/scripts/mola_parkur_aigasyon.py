...
class TeknofestGPSMission(Node):
    def __init__(self):
        ...
        self.kp_steering = 2.5
        self.ki_steering = 0.02
        self.kd_steering = 5.0

        self.pivot_threshold = 1.57
        self.align_threshold = 0.79
        self.approach_distance = 5.0
        self.min_approach_angle = 0.61
    ...
    def control_loop(self):
        ...
        if self.state == STATE_INIT_GPS:
            ...
            if lidar_angle is not None:
                ...
                waypoint_path_blocked = (lidar_obstacle_dist < 2.5) and (abs(gps_err) < 0.26)
            ...
            if abs(gps_err) > self.pivot_threshold:
            ...
            elif abs(gps_err) > self.align_threshold:
                cmd.linear.x = 0.15
                turn_multiplier = 1.3
            ...
            if len(self.waypoints_xy) > 4:
                ...
                if self.parkur2_started and target_dist < 2.0:
                    self.get_logger().warn(
                        "="*60 + "\n" +
