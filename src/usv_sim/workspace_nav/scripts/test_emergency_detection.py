#!/usr/bin/env python3
"""
TEKNOFEST USV - REFLEXIVE PARKOUR NAVIGATOR
═══════════════════════════════════════════════════════════════════════════════
Reflexive Navigation: Reactive + Predictive Hybrid

Özellikler:
✅ Adaptive Lookahead (Engele göre dinamik bakış mesafesi)
✅ Side Repulsion (Drift önleyici yan itme kuvveti)
✅ Virtual Centerline & Pure Pursuit
✅ Smart Speed Control (Engel yakınlığına göre)
✅ Emergency Recovery (3-fazlı)
═══════════════════════════════════════════════════════════════════════════════
"""
...
# ═══════════════════════════════════════════════════════════════════════════
# REFLEXIVE NAVIGATION PARAMETERS (Optimized v3)
# ═══════════════════════════════════════════════════════════════════════════

# 1. ADAPTIVE LOOKAHEAD (Engele göre dinamik)
NORMAL_LOOKAHEAD = 4.5    # Engel uzakta (> 3.0m) - MAX_LOOKAHEAD
PANIC_LOOKAHEAD = 3.0     # Engel çok yakın (< 2.5m) - MIN_LOOKAHEAD (erken algılama)
OBSTACLE_FAR = 3.0        # "Uzak" threshold (m)
OBSTACLE_NEAR = 2.5       # "Yakın" threshold (m)
LOOKAHEAD_GAIN = 1.0      # Hıza bağlı lookahead çarpanı
FRONT_CONE_ANGLE = 30     # Ön algılama konisi (derece)

# 2. SIDE REPULSION (Drift önleyici)
SIDE_ZONE_X_MIN = 0.0     # Yan bölge başlangıcı (m)
SIDE_ZONE_X_MAX = 1.5     # Yan bölge sonu (m)
SIDE_ZONE_Y_MAX = 2.0     # Yan bölge genişliği (m)
SIDE_REPULSION_GAIN = 2.0 # Yan itme kuvveti çarpanı

# 3. CORNER BIAS (Virajlarda dubalardan uzak dur)
CORNER_BIAS_GAIN = 0.8    # Viraj içine çekme kuvveti

# 4. KANAL & VİRAJ
CHANNEL_WIDTH = 5.0       # İki duba arası genişlik (m)

# 5. HIZ & DÖNÜŞ
MAX_SPEED = 2.0           # Maksimum hız (m/s)
MIN_SPEED = 0.8           # Minimum hız (m/s) - virajda yavaş
TURN_GAIN = 1.3           # Pure Pursuit gain - dengeli
MAX_ANGULAR_VEL = 1.5     # Maksimum açısal hız (rad/s)

# 6. SMOOTHING
SMOOTHING_FACTOR = 0.35   # EMA faktörü - dengeli
...
# 7. CHECKPOINT (Duba geçişi sonrası)
CHECKPOINT_STOP_DURATION = 2.0    # Durma süresi (saniye) - artırıldı
CHECKPOINT_ALIGN_DURATION = 4.0   # Maksimum dönme süresi (saniye) - artırıldı
CHECKPOINT_HEADING_THRESHOLD = 0.1 # Heading toleransı (radyan ~5.7°)
BUOY_PASSAGE_Y_THRESHOLD = 0.3    # Y koordinatı değişim eşiği (m)

# 8. KAMIKAZE ACTIVATION
KAMIKAZE_ACTIVATION_DISTANCE = 15.0  # WP5'e bu mesafeden yakınsa kamikaze aktif (m) - artırıldı
KAMIKAZE_FORCE_ACTIVATION_DISTANCE = 10.0  # Bu mesafeden yakınsa hybrid olmasa da aktif et (m)
KAMIKAZE_TARGET_AREA_MIN = 1500      # Minimum hedef alanı (piksel²)
...
STATE_CHECKPOINT_STOP = 10   # Duba geçişi sonrası durma
STATE_CHECKPOINT_ALIGN = 11  # Waypoint'e dönme


class ReflexiveParkourNode(Node):
    """
    Reflexive Parkour Navigator with:
    - Adaptive Lookahead (Early Reaction)
    - Side Repulsion (Drift Prevention)
    - Virtual Centerline & Pure Pursuit
    """
    ...
    def __init__(self):
        ...
        # Buoy Checkpoint Tracking
        self.buoy_tracking = {
            'last_nearest_y': None,      # Son en yakın engelin Y pozisyonu
            'last_nearest_x': None,      # Son en yakın engelin X pozisyonu
            'passage_detected': False,   # Geçiş algılandı mı?
            'checkpoint_start_time': 0.0 # Checkpoint başlangıç zamanı
        }
        
        # Hybrid Section Tracking
        self.hybrid_section_complete = False  # Hybrid parkour bölümü tamamlandı mı?
        
        # Kamikaze
        self.kamikaze_detected = False
        self.kamikaze_last_seen = 0.0
        self.kamikaze_target_area = 0.0
        self.kamikaze_target_x = 0.0  # Hedef X pozisyonu (piksel, kamera merkezi = 0)
        self.kamikaze_target_y = 0.0  # Hedef Y pozisyonu (piksel, kamera merkezi = 0)
    ...
    def check_emergency_condition(self, points):
        ...
        # DEBUG: Safety Box içindeki nokta sayısı
    ...
    # ═══════════════════════════════════════════════════════════════════════
    # BUOY PASSAGE DETECTION
    # ═══════════════════════════════════════════════════════════════════════
    
    def detect_buoy_passage(self, points):
        """
        Duba geçişini algıla (lateral position tracking)
        Returns: (passage_detected: bool, buoy_side: str)
        """
        if len(points) == 0:
            return False, None
        
        # En yakın engeli bul (önümüzde, 3m içinde)
        ...
        # Y koordinatı işaret değiştirdi mi? (geçiş!)
        last_y = self.buoy_tracking['last_nearest_y']
        
        # Sol duba geçişi: pozitif → negatif
        if last_y > BUOY_PASSAGE_Y_THRESHOLD and current_y < -BUOY_PASSAGE_Y_THRESHOLD:
            self.buoy_tracking['last_nearest_y'] = None  # Reset for next buoy
            return True, "SOL"
        
        # Sağ duba geçişi: negatif → pozitif
    ...
    # ═══════════════════════════════════════════════════════════════════════
    # REFLEXIVE NAVIGATION: ADAPTIVE LOOKAHEAD + SIDE REPULSION
    # ═══════════════════════════════════════════════════════════════════════
    
    def calculate_reflexive_steering(self, points):
        """
        Reflexive Navigation Algorithm:
        1. Adaptive Lookahead (engele göre dinamik)
        2. Speed-Based Lookahead (hıza göre ek ayar)
        3. Virtual Centerline (Pure Pursuit)
        4. Corner Bias (virajlarda dubalardan uzak)
        5. Side Repulsion (drift önleyici)
        6. Final Mixing
        """
        if len(points) == 0:
        ...
        # ───────────────────────────────────────────────────────────────────
        # 1. ADAPTIVE LOOKAHEAD (Early Reaction System)
        # ───────────────────────────────────────────────────────────────────
        # Önümüzdeki engelleri bul (±FRONT_CONE_ANGLE derece)
        ...
        # Base lookahead'i engel mesafesine göre ayarla
        ...
        # 2. SPEED-BASED LOOKAHEAD (Hıza göre ek ayar)
        # ───────────────────────────────────────────────────────────────────
        # Yüksek hızda daha uzağa bak, düşük hızda daha yakına
        ...
        if has_left and has_right:
            ...
            # ───────────────────────────────────────────────────────────────
            # 4. CORNER BIAS (Virajlarda dubalardan uzak dur)
            # ───────────────────────────────────────────────────────────────
            # Viraj içine çek (dubalardan uzaklaş)
            if target_y > 0:  # Sol viraj
        ...
        # ───────────────────────────────────────────────────────────────────
        # 5. SIDE REPULSION (Snap-Back Mechanism)
        # ───────────────────────────────────────────────────────────────────
        ...
        if len(side_points) > 0:
            for point in side_points:
                py = point[1]
                # Repulsion = 1 / (distance + epsilon)
                force = 1.0 / (abs(py) + 0.1)
                
                # Yön: Sağda engel varsa sola it (negatif), solda varsa sağa it (pozitif)
                if py > 0:  # Sol tarafta engel
                    repulsion_force -= force
                else:  # Sağ tarafta engel
                    repulsion_force += force
        
        # ───────────────────────────────────────────────────────────────────
        # 6. FINAL STEERING MIXING
        # ───────────────────────────────────────────────────────────────────
        # Pure Pursuit (1.0x) + Side Repulsion (2.0x)
        final_curvature = (curvature_pp * 1.0) + (repulsion_force * SIDE_REPULSION_GAIN)
        
        # ───────────────────────────────────────────────────────────────────
        # 7. SMART SPEED CONTROL
        # ───────────────────────────────────────────────────────────────────
        if nearest_obstacle < 2.0 or abs(final_curvature) > 0.5:
            target_speed = MIN_SPEED
        else:
            # Linear speed reduction based on curvature
    ...
    def kamikaze_callback(self, msg):
        """
        Kırmızı duba algılama callback'i
        msg.x = hedef X pozisyonu (normalized 0-1)
        msg.y = hedef Y pozisyonu (normalized 0-1)
        msg.z = hedef alanı (piksel²)
        """
        self.kamikaze_last_seen = time.time()
        
        # Convert normalized coordinates (0-1) to pixel-based (center = 0)
        CAMERA_WIDTH = 640
        CAMERA_HEIGHT = 480
        
        # msg.x is normalized (0-1), convert to pixel offset from center
        self.kamikaze_target_x = (msg.x - 0.5) * CAMERA_WIDTH  # -320 to +320
        self.kamikaze_target_y = (msg.y - 0.5) * CAMERA_HEIGHT  # -240 to +240
        ...
        # Yeterince büyük hedef algılandı
    ...
    def control_loop(self):
        ...
        if self.state == STATE_PARKUR_2_GAP:
            ...
            # DEBUG: Goal distance'u periyodik logla
            ...
            # Kamikaze aktivasyon kontrolü
            # Koşul 1: Hybrid tamamlandı VE normal zone'da (<15m)
            # Koşul 2: Çok yakın (<10m) - hybrid olmasa da aktif et
            ...
            if self.latest_scan is None:
                ...
                # Güvenli hız kontrolü (çok hızlı gitme)
            ...
            # ═══════════════════════════════════════════════════════════════
            # BUOY PASSAGE DETECTION & CHECKPOINT TRIGGER
            # ═══════════════════════════════════════════════════════════════
            ...
            if not self.emergency_active:
                ...
                if passage:
                    self.hybrid_section_complete = True  # Mark hybrid section as complete
                    self.get_logger().warn(
            ...
            if result is not None and result[0] is not None:
                ...
                # Gelişmiş logging (her 1 saniyede bir - 20Hz)
                self._log_counter += 1
        ...
        # ═══════════════════════════════════════════════════════════════════════
            ...
            # Heading'e yeterince yakınsak, navigasyona devam et
            if abs(gps_heading) < CHECKPOINT_HEADING_THRESHOLD:
                self.state = STATE_PARKUR_2_GAP
                self.buoy_tracking['last_nearest_y'] = None  # Reset tracking
        ...
        # ═══════════════════════════════════════════════════════════════════════
        # PARKUR 3 - KAMIKAZE ATTACK (Direct Vision Control)
        # ═══════════════════════════════════════════════════════════════════════
        
        elif self.state == STATE_PARKUR_3_ATTACK:
            # Kamikaze hedefi varsa, ona hizalan ve saldır
            if (current_time - self.kamikaze_last_seen) < 1.0:
                # Hedef görülüyor
                target_x = self.kamikaze_target_x  # Piksel cinsinden sapma (merkez = 0)
                target_area = self.kamikaze_target_area
                
                # Kamera genişliği varsayımı: 640 piksel
                # X sapması: -320 (sol) ile +320 (sağ) arası
                # Normalize et: -1.0 (sol) ile +1.0 (sağ)
                CAMERA_WIDTH = 640
                normalized_x = target_x / (CAMERA_WIDTH / 2.0)  # -1.0 ile +1.0
                
                # Hizalama kontrolü (P kontrolcü)
                ALIGNMENT_GAIN = 1.5  # Dönüş kazancı
                angular_z = -normalized_x * ALIGNMENT_GAIN  # Negatif çünkü sağa gitmek için - dönüş
                
                if target_area > KAMIKAZE_TARGET_AREA_MIN:
                    # Hedef yeterince büyük, MAKSIMUM HIZ SALDIRI!
                    KAMIKAZE_MAX_SPEED = 3.0  # Kamikaze için özel yüksek hız
...
#ULTİMATE ESKİ HAL GEMİNİ ÖNCESİ
