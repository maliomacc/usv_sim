...
void WaveVisualPrivate::OnUpdate()
{
  if (this->visualName.empty()) {
    ignwarn << "WaveVisual: visualName is empty." << std::endl;
    return;
  }
  ...
  if (!this->visual)
  {
    ...
    while (!nodes.empty())
    {
      ...
      if (n && n->HasUserData("gazebo-entity"))
      {
        ...
        const unsigned int *uvalue = std::get_if<unsigned int>(&variant);
        uint64_t entityId = 0;
        if (value) entityId = static_cast<uint64_t>(*value);
        else if (uvalue) entityId = static_cast<uint64_t>(*uvalue);

        if ((value || uvalue) && entityId == static_cast<uint64_t>(this->entity))
        {
  ...
  if (!this->visual) {
    ignwarn << "WaveVisual: Could not find visual for entity " << this->entity << std::endl;
    return;
  }
  ...
  if (!this->paramsSet)
  {
    ...
    std::string bumpMapPath = common::findFile("model://waves/materials/textures/wave_normals.dds");
    if (bumpMapPath.empty()) {
        ignerr << "Failed to find wave_normals.dds" << std::endl;
    }
    (*fsParams)["bumpMap"].SetTexture(bumpMapPath,
        rendering::ShaderParam::ParamType::PARAM_TEXTURE);

    std::string cubeMapPath = common::findFile("model://waves/materials/textures/skybox_lowres.dds");
    if (cubeMapPath.empty()) {
        ignerr << "Failed to find skybox_lowres.dds" << std::endl;
    }
