    ...
    const msgs::StringMsg_V &_msg)
{
  if (_msg.data_size() < 2)
  {
    ignerr << "2 string values, [shape, color], are required "
...
bool PlacardPlugin::Implementation::ParseSDF(sdf::ElementPtr _sdf)
{
  ...
  if (_sdf->HasElement("shape"))
  {
    ...
    {
      ignerr << "Incorrect " << aShape << " <shape>, using random shape"
            << std::endl;
    }
  }
  ...
  if (_sdf->HasElement("color"))
  {
    ...
    {
      ignerr << "Incorrect " << aColor << " <color>, using random color"
            << std::endl;
    }
  }
  ...
  if (_sdf->HasElement("shuffle"))
  {
    ...
    {
      ignerr << "<shuffle_topic> missing" << std::endl;
      return false;
    }
  ...
  {
    ignerr << "<robot_namespace> missing" << std::endl;
    return false;
  }
...
void PlacardPlugin::Implementation::ChangeSymbol(const msgs::Empty &_msg)
{
  ...
  ignmsg << "New symbol is " << this->color << " " << this->shape << std::endl;
}

IGNITION_ADD_PLUGIN(PlacardPlugin,
              sim::System,
              PlacardPlugin::ISystemConfigure)

IGNITION_ADD_PLUGIN_ALIAS(vrx::PlacardPlugin,
                    "vrx::PlacardPlugin")
