...
{
  ignmsg << "AcousticPerceptionScoringPlugin scoring plugin loaded" << std::endl;
}
  ...
  sim::EntityComponentManager &_ecm, sim::EventManager &_eventMgr)
{
  ...
  {
    ignerr << "Unable to find <pinger_position>" << std::endl;
    return;
  }
...
IGNITION_ADD_PLUGIN(vrx::AcousticPerceptionScoringPlugin,
              sim::System,
