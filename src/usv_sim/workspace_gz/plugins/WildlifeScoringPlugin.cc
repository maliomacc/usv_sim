    ...
    ecm(_ecm)
{
  ...
  for (int i = 0; i < this->kNumVirtualGates; ++i)
  {
    ...
    if (!comp)
    {
      ignerr << "Unable to find component WorldPose for entity ["
...
void Buoy::Update()
{
  ...
  if (!comp)
  {
    ignerr << "Unable to find Pose for vehicle model [" << this->vehicleModel
          << "]" << std::endl;
    return;
  }
  ...
  if (!comp2)
  {
    ignerr << "Unable to find WorldPose for buoy [" << this->entity
          << "]" << std::endl;
    return;
  }
  ...
  if (this->state == BuoyState::NEVER_ENGAGED)
  {
    if (vehicleBuoyDistance <= this->engagementDistance)
    {
      ...
      igndbg << this->entityName << " Transition from NEVER_ENGAGED"
            << " to ENGAGED" << std::endl;
      std::cout << std::flush;
      return;
    }
  ...
  else if (this->state == BuoyState::NOT_ENGAGED)
  {
    if (vehicleBuoyDistance <= this->engagementDistance)
    {
      // Transition to ENGAGED.
      this->state = BuoyState::ENGAGED;
      igndbg << this->entityName << " Transition from NOT_ENGAGED"
            << " to ENGAGED" << std::endl;
      std::cout << std::flush;
      return;
    }
  ...
  else if (this->state == BuoyState::ENGAGED)
  {
    if (vehicleBuoyDistance > this->engagementDistance)
    {
      ...
      igndbg << this->entityName << " Transition from ENGAGED"
  ...
  sim::EntityComponentManager &_ecm)
{
  ...
  while (buoyElem)
  {
    ...
    {
      ignerr << "Unable to find <buoys><buoy><name> element in SDF."
            << std::endl;
      return false;
    }
  ...
  const sim::UpdateInfo &_info, sim::EntityComponentManager &_ecm)
{
  ...
  for (auto const &buoy : this->buoys)
  {
    ...
    if (!comp)
    {
      ignerr << "Unable to find WorldPose component for entity ["
...
IGNITION_ADD_PLUGIN(WildlifeScoringPlugin,
              sim::System,
              WildlifeScoringPlugin::ISystemConfigure,
              WildlifeScoringPlugin::ISystemPreUpdate)

IGNITION_ADD_PLUGIN_ALIAS(vrx::WildlifeScoringPlugin,
