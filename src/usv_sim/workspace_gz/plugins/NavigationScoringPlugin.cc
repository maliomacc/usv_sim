    ...
    sim::EntityComponentManager &_ecm)
{
  ...
  {
    ignerr << "Unable to find entity [" << this->leftMarkerName << "]"
          << std::endl;
    return false;
  }
  ...
  {
    ignerr << "Unable to find entity [" << this->rightMarkerName << "]"
          << std::endl;
    return false;
  }
  ...
  sim::EntityComponentManager &_ecm, sim::EventManager &_eventMgr)
{
  ...
  {
    ignerr << "Unable to parse gates. Score has been disabled" << std::endl;
    return;
  }
  ...
  if (!_sdf->HasElement("bonus"))
  {
    ignerr << "Unable to find <bonus> element in SDF." << std::endl;
    return;
  }
  ...
  sim::EntityComponentManager &_ecm)
{
  ...
  {
    ignmsg << "Loading " << this->dataPtr->numGates << " gates." << std::endl;
  ...
  if (newCollisions > 0)
  {
    ...
    igndbg << "New penalty. score: " << this->Score() << std::endl;
    std::cout << std::flush;
  }
       ...
       i < this->dataPtr->numGates; ++i)
  {
    ...
    {
      gate.state = Implementation::GateState::CROSSED;
      igndbg << "New gate crossed!" << std::endl;
      std::cout << std::flush;
      ...
      {
        igndbg << "  Consecutive gate bonus!" << std::endl;
      ...
      igndbg << "Score: " << this->Score() << std::endl;
      std::cout << std::flush;
      ...
      {
        this->SetScore(this->Score() + this->dataPtr->bonusConsecutiveGate);
        igndbg << "Extra bonus for completing the course" << std::endl;
        igndbg << "Score: " << this->Score() << std::endl;
        igndbg << "Course completed!" << std::endl;
        std::cout << std::flush;
              ...
              gate.state   == Implementation::GateState::VEHICLE_AFTER))
    {
      ...
      igndbg << "Transited the gate in the wrong direction. Gate invalidated!"
            << std::endl;
      std::cout << std::flush;
