...
{
  ignmsg << "ColorSequenceChecker: Color sequence submission received:"
  ...
  {
    ignerr << "The color sequence has already been submitted" << std::endl;
    return;
  }
  ...
  {
    ignerr << "Received a color sequence with " << _sequence.data_size()
  ...
  {
    ignmsg << "ColorSequenceChecker: Received color sequence is correct. "
          << " Additional points will be scored." << std::endl;
  }
  else
  {
    ignmsg << "ColorSequenceChecker: Received color sequence is not correct. "
    ...
    vehicleName(_vehicleName)
{
  ...
  {
    ignerr << "Error subscribing to topic [" << this->internalActivationTopic
           << "]" << std::endl;
    return;
  }
  ...
  {
    ignerr << "Error subscribing to topic [" << this->externalActivationTopic
           << "]" << std::endl;
    return;
  }
}
...
void DockChecker::Update(const sim::UpdateInfo &_info)
{
  ...
  if (this->anytimeDocked)
  {
    ignmsg  << "Successfully stayed in dock for " << this->minDockTime.count()
...
void DockChecker::OnInternalActivationEvent(const msgs::Pose &_msg)
{
  ...
  if (state == "1")
  {
    ...
    ignmsg << "Entering internal dock activation zone, transitioning to "
  ...
  if (state == "0")
  {
    ...
    if (this->AnytimeDocked())
    {
      ignmsg << "Leaving internal dock activation zone in [" << this->name
  ...
  igndbg << "[" << this->name << "] OnInternalActivationEvent(): "
        << state << std::endl;
  std::cout << std::flush;
}
...
void DockChecker::OnExternalActivationEvent(const msgs::Pose &_msg)
{
  ...
  if (this->atEntrance)
  {
    ignmsg << "Entering external dock activation zone in [" << this->name
          << "]" << std::endl;
  }
  else
  {
    ignmsg << "Leaving external dock activation zone in [" << this->name
          << "]" << std::endl;
  }

  igndbg << "[" << this->name << "] OnExternalActivationEvent(): "
        << state << std::endl;
  std::cout << std::flush;
}
  ...
  const std::string _vehicleName)
{
  ...
  for (auto colorIndex : {"color_1", "color_2", "color_3"})
  {
    ...
    {
      ignerr << "ScanDockScoringPlugin: Missing <" << colorIndex << ">"
            << std::endl;
      return false;
    }
    ...
    {
      ignerr << "ScanDockScoringPlugin:: Invalid color [" << color << "]"
            << std::endl;
      return false;
    }
  ...
  while (bayElem)
  {
    ...
    {
      ignerr << "<bays::bay::name> missing" << std::endl;
      return false;
    }
    ...
    {
      ignerr << "<bays::bay::internal_activation_topic> missing" << std::endl;
      return false;
    }
    ...
    {
      ignerr << "<bays::bay::external_activation_topic> missing" << std::endl;
      return false;
    }
    ...
    {
      ignerr << "<bays::bay::symbol_topic> missing" << std::endl;
      return false;
    }
    ...
    {
      ignerr << "<bays::bay::min_dock_time> missing" << std::endl;
      return false;
    }
    ...
    {
      ignerr << "<bays::bay::correct_dock> missing" << std::endl;
      return false;
    }
    ...
    {
      ignerr << "<bays::bay::symbol> not found" << std::endl;
    }
  ...
  if (_sdf->HasElement("targets"))
  {
    ...
    {
      ignerr << "<targets><target> not found" << std::endl;
      return false;
    }
    ...
    while (targetElem)
    {
      ...
      {
        ignerr << "<targets><target><topic> not found" << std::endl;
        return false;
      }
      ...
      {
        ignerr << "<targets><target><bonus_points> not found" << std::endl;
        return false;
      }
  ...
  sim::EntityComponentManager &_ecm, sim::EventManager &_eventMgr)
{
  ...
  {
    ignerr << "Error parsing SDF, plugin disabled." << std::endl;
    return;
  }
}
  ...
  sim::EntityComponentManager &_ecm)
{
  ...
  if (this->dataPtr->enableColorChecker)
  {
        ...
        this->dataPtr->colorChecker->SubmissionReceived())
    {
      ...
      {
        ignmsg << "Adding <" << this->dataPtr->colorBonusPoints
  ...
  for (auto &dockChecker : this->dataPtr->dockCheckers)
  {
    ...
    {
      ignmsg  << "Successfully docked in [" << dockChecker->name << "]"
    ...
    if (dockChecker->Correct())
    {
      ...
      if (this->TaskState() == "running")
      {
        ignmsg << "Docked in correct dock [" << dockChecker->name << "]"
    ...
    else
    {
      if (this->TaskState() == "running")
      {
        ignmsg  << "Docked in incorrect dock [" << dockChecker->name << "]"
               << ". No additional points." << std::endl;
      }
    }
...
IGNITION_ADD_PLUGIN(ScanDockScoringPlugin,
              sim::System,
              ScanDockScoringPlugin::ISystemConfigure,
              ScanDockScoringPlugin::ISystemPreUpdate)

IGNITION_ADD_PLUGIN_ALIAS(vrx::ScanDockScoringPlugin,
