    ...
    ecm(_ecm)
{
  ...
  if (entities.empty())
  {
    ignerr << "Unable to find entity [" << _name << "]" << std::endl;
    return;
  }
...
void PerceptionObject::StartTrial(const sim::Entity _frame)
{
  ...
  igndbg << "PerceptionScoringPlugin: spawning " << this->name << std::endl;
}
...
void PerceptionObject::EndTrial()
{
  ...
  igndbg << "PerceptionScoringPlugin: despawning " << this->name << std::endl;
}
  ...
  sim::EntityComponentManager &_ecm)
{
  ...
  {
    ignerr << "PerceptionScoringPlugin: Unable to find <object_sequence> element"
          << std::endl;
    return false;
  }
  ...
  while (objectElem)
  {
    ...
    {
      ignerr << "PerceptionScoringPlugin: Unable to find <time> in object\n";
    ...
    {
      ignerr << "PerceptionScoringPlugin: Unable to find <type> in object.\n";
      objectElem = objectElem->GetNextElement("object");
      continue;
    }
    ...
    {
      ignerr << "PerceptionScoringPlugin: Unable to find <name> in object.\n";
      objectElem = objectElem->GetNextElement("object");
      continue;
    }
    ...
    {
      ignerr << "PerceptionScoringPlugin: Unable to find <pose> in object.\n";
      objectElem = objectElem->GetNextElement("object");
      continue;
    }
    ...
    igndbg << "New object: " << std::endl << obj << std::endl;
  ...
  sim::EntityComponentManager &_ecm)
{
  ...
  for (auto &_msg : this->requests)
  {
    ...
    {
      ignwarn << "PerceptionScoring: Attempt Balance is 0, no attempts currently"
    ...
    // Burn one attempt.
    --this->attemptBal;
    igndbg << "PerceptionScoring: New Attempt Balance: " << this->attemptBal
          << std::endl;
  ...
  sim::EntityComponentManager &_ecm)
{
  ...
  if (this->dataPtr->firstIteration)
  {
    ...
    {
      ignerr << "Perception scoring disabled" << std::endl;
      return;
    }
  }
  ...
  for (auto &obj : this->dataPtr->objects)
  {
        ...
        this->ElapsedTime().count() < obj.time + obj.duration && !obj.active)
    {
      ...
      igndbg << "PerceptionScoring: New Attempt Balance: "
      ...
      this->TaskState() != "finished")
  {
    ...
    for (auto &obj : this->dataPtr->objects)
      igndbg << "PerceptionScoring: " << std::endl << obj << std::endl;

    // Run score is the mean error per object.
    this->SetScore(this->Score() / this->dataPtr->objects.size());
    igndbg << "Perception run score: " << this->Score() << std::endl;
...
IGNITION_ADD_PLUGIN(PerceptionScoringPlugin,
              sim::System,
              PerceptionScoringPlugin::ISystemConfigure,
              PerceptionScoringPlugin::ISystemPreUpdate)

IGNITION_ADD_PLUGIN_ALIAS(vrx::PerceptionScoringPlugin,
