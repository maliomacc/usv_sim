...
class LightBuoyPlugin::Implementation
{
  ...
  /// \brief Creates a ignition::msgs::Color message from 4 doubles.
  /// \param[in] _r Red.
  /// \param[in] _g Green.
  /// \param[in] _b Blue.
  /// \param[in] _a Alpha.
...
std::map<std::string, ignition::msgs::Color>
...
bool LightBuoyPlugin::Implementation::ParseSDF(sdf::ElementPtr _sdf)
{
  ...
  for (auto colorIndex : {"color_1", "color_2", "color_3"})
  {
    ...
    {
      ignerr << "Missing <" << colorIndex << ">" << std::endl;
      return false;
    }
    ...
    {
      ignerr << "Invalid color [" << color.c_str() << "]" << std::endl;
      return false;
    }
  ...
  {
    ignerr << "<visuals> missing" << std::endl;
    return false;
  }
  ...
  {
    ignerr << "<visual> missing" << std::endl;
    return false;
  }
  ...
  if (this->visuals.empty())
  {
    ...
    while (!nodes.empty())
    {
      ...
      if (n && n->HasUserData("gazebo-entity"))
      {
        ...
        const unsigned int *value = std::get_if<unsigned int>(&variant);
    ...
    if (!visual)
    {
      ignerr << "Unable to find visual associated with entity id: "
    ...
    if (!linkVisual)
    {
      ignerr << "Unable to find parent link associated with visual entity id: "
            << this->entity << std::endl;
      return;
    }
    ...
    for (const auto &name : this->visualNames)
    {
      ...
      {
        ignerr << "Unable to find visual: " << name << std::endl;
      }
    }
  }
...
IGNITION_ADD_PLUGIN(LightBuoyPlugin,
              sim::System,
              LightBuoyPlugin::ISystemConfigure,
              LightBuoyPlugin::ISystemPreUpdate)

IGNITION_ADD_PLUGIN_ALIAS(vrx::LightBuoyPlugin,
                    "vrx::LightBuoyPlugin")
