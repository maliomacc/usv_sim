...
class TeknofestGPSMission(Node):
    def __init__(self):
        ...
        # === TUNING AYARLARI ===
        self.declare_parameter('max_speed', 2.0)        # Hızı artırdık
        self.declare_parameter('waypoint_tolerance', 2.0) # Hızlı giderken toleransı biraz artırmak iyidir
        
        # PD Katsayıları (Daha çevik dönüş için)
        self.kp_steering = 2.5   # Oransal kazanç (Artırıldı: Daha hızlı tepki)
        self.kd_steering = 0.5   # Türev kazanç (Yeni: Hedefi aşmayı engeller)
    ...
    def control_loop(self):
        ...
        # --- STATE 1: PARKUR 1 (GPS / JSON) ---
            ...
            if self.wp_index >= len(self.waypoints_xy):
                ...
                self.get_logger().info("PARKUR 1 BİTTİ -> PARKUR 2 (LIDAR) BAŞLIYOR")
                return
            ...
            # --- GELİŞMİŞ PD KONTROL ---
            # Hata türevi (D terimi) - Savrulmayı önler
            d_error = (gps_err - self.prev_error) / 0.05 # 20Hz döngü süresine bölüyoruz
            
            # Dönüş komutu: P ve D ağırlıklarını burada dengeleyebilirsin
            # Eğer hala geç dönüyorsa self.kp_steering değerini 3.0 veya 3.5 yap.
            turn_cmd = (self.kp_steering * gps_err) + (self.kd_steering * d_error)
            
            # Engel kontrolü (Lidar önceliği)
            ...
            # Maksimum dönüş hızını 2.0'a kadar çekebilirsin (Daha kıvrak olur)
            cmd.angular.z = max(min(turn_cmd, 1.8), -1.8) 
            self.prev_error = gps_err

            # --- HIZ TUNING (Kritik Nokta) ---
            # Eğer açı hatası çok fazlaysa (robot hedefe arkasını dönmüşse vb.) 
            # ilerlemek yerine olduğu yerde dönmesini sağla.
            if abs(gps_err) > 1.0: # Yaklaşık 60 derece
                cmd.linear.x = 0.2  # Olduğu yerde dön
            elif abs(gps_err) > 0.4: # Yaklaşık 23 derece
                cmd.linear.x = max_vel * 0.3 # Yavaşla ve dön
            else:
                # Yol düzse ama hedefe çok yakınsak waypoint'i ıskalamamak için yavaşla
                if dist < 5.0:
        ...
        # STATE 2: PARKUR 2 (LIDAR)
            ...
            if angle:
                ...
                cmd.linear.x = max_vel if abs(angle) < 0.3 else max_vel * 0.5
            else:
                cmd.linear.x = -0.2
                cmd.angular.z = 0.8
